...
**[tendril](https://github.com/moloo4ni/tendril)** — scroll-driven wayland compositor in smithay + rust. fixed two-column layout with per-column vertical strip scrolling — no floating windows or stacking. nested compositor (winit), xdg-shell, xdg-decoration, popup surfaces. smooth animation (lerp scroll, persistent z-effect fade). ipc: json-rpc 2.0 over unix socket + `tendrilc` cli (list-windows with titles, get-config). 5 workspaces, keyboard navigation (hjkl), z-effect reorder, toml config, column balancing, damage-aware rendering.
