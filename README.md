...
**[tendril](https://github.com/moloo4ni/tendril)** — scroll-driven wayland compositor in smithay + rust. fixed two-column layout with per-column vertical strip scrolling — no floating windows or stacking. nested compositor (winit), xdg-shell, xdg-decoration. ipc: json-rpc 2.0 over unix socket + `tendrilc` cli. 5 workspaces, keyboard navigation (hjkl) with z-effect reorder, toml config, column balancing.
